...
'''
ERA_VAR = 'potential_evaporation'
VAR_NAME = 'pev'
PRODUCT_NAME = 'era5_reanalysis_pev_daily'
...
def download_and_cog_era5(
    ...
    for count_cds, value_cds in enumerate(era_data):
        ...
        for count, value in enumerate(lst_time):
            ...
            precip_date = value - np.timedelta64(1, 'D') # this is needed only for var with accumulation values - see ERA5 Land Accumulation
                        
            # Read nc file as xarray and select one timestamp at the time
            # prep_array = era_data[count_cds].to_xarray()
            ...
            file_name = np.datetime_as_string(precip_date, unit='h')
            file_name = file_name.replace('-', "")
            array_tif[VAR_NAME].rio.to_raster(f'temp/{ERA_VAR}_{file_name}.tif')
            
            # Set variables for file names
            y = pd.to_datetime(precip_date).year
            m = pd.to_datetime(precip_date).month
            d = pd.to_datetime(precip_date).day
            h = pd.to_datetime(precip_date).hour
            
            file_base = f"{s3_dst}/{y}/{m}/era5-reanalysis-pev-daily.{y}."\
            ...
            try:
                ...
                with MemoryFile() as mem_dst:
                    ...
                    item = create_stac_item(
                        mem_dst,
                        id=str(odc_uuid("era5-land", "5.0", [f'{ERA_VAR}_{file_name}.tif'])),
                        with_proj=True,
                        input_datetime=pd.to_datetime(value),
                        properties={
                    ...
                    item.assets["potential_evaporation"] = pystac.Asset(
                        href=out_data,
...
@click.option("--end_date", default="1980-01-01",
              help="the end date needs to be n+1 due to the way ERA5 Land data is retrieved")
...
def reanalysis_era5_land(
    ...
    # This will do a whole year for different peridos in differen years
    # For periods with the same year, it will do the months asked for
    
    if(start_date.year != end_date.year):
        # Create list of years, month, day and 

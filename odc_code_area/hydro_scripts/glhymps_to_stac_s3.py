...
log.info("Starting ghlymps uploader")
...
def download_and_cog_soils(
    ...
    for count_cds, value_soil in enumerate(soil_data):
        ...
        end_datetime = "2018-12-31T11:59:59Z"
        product_name = f"ghlymps_{lst_name[0]}_{lst_name[1].replace('-','_')}"
        in_data = value_soil
 
        
        # file base with depth folder included
        file_base = f"{s3_dst}/{lst_name[0]}/{lst_name[1]}"\
        f"/ghlymps_{file_name.split('.')[0]}"
        print(file_base)
        ...
        try:
            ...
            with MemoryFile() as mem_dst:
                ...
                item = create_stac_item(
                    mem_dst,
                    id=str(odc_uuid("GLHYMPS", "2.0", [f'{lst_name[0]}_{file_name}'])),
                    with_proj=True,
                    input_datetime=pd.to_datetime("2018-01-01T00:00:00"),
                    properties={
                ...
                item.assets[f'{lst_name[0]}'] = pystac.Asset(
                    href=out_data,
                    title=f"GLHYMPS_2.0_{file_name.split('.')[0]}",
                ...
                item.add_links(
                    [
                        pystac.Link(
                            target='https://borealisdata.ca/dataset.xhtml?persistentId=doi%3A10.5683/SP2/TTJNIU',
                            title="Source file",
...
#############################################################
@click.command("uploading GLHYMPS to S3")
...
def hihydrosoils(
    ...
    download_and_cog_soils(files_path, s3_dst)

...
VAR_NAME = 't2m'
PRODUCT_NAME = 'era5_reanalysis_tmean_daily'
...
def cds_response(variable_name, year_data, month_data, day_data, lst_horas):
    ...
    source_prep = cml.load_source(
        ...
        # area=[13, -79, -4.5, -66], #[latmean, longmin, latmin, lonmax] This is for Colombia
...
def download_and_cog_era5(
    ...
    for count_cds, value_cds in enumerate(era_data):
        ...
        with xr.open_dataset(value_cds) as ds:
            ...
            # Converting t2m from K to C and get the daily value
            array_data = ds.resample(time='1D').mean() - 273.15
        ...
        for count, value in enumerate(lst_time):
            ...
            file_name = np.datetime_as_string(temp_date, unit='h')
            file_name = file_name.replace('-', "")
            array_tif[VAR_NAME].rio.to_raster(f'temp/{ERA_VAR}_{file_name}.tif')
            
            # Set variables for file names
            y = pd.to_datetime(temp_date).year
            m = pd.to_datetime(temp_date).month
            d = pd.to_datetime(temp_date).day
            h = pd.to_datetime(temp_date).hour
            
            file_base = f"{s3_dst}/{y}/{m}/era5-reanalysis-tmean-daily.{y}."\
            ...
            try:
                ...
                with MemoryFile() as mem_dst:
                    ...
                    item = create_stac_item(
                        mem_dst,
                        id=str(odc_uuid("era5-land", "5.0", [f'{ERA_VAR}_{file_name}_tmean.tif'])),
                        with_proj=True,
                    ...
                    item.assets["2m_temperature_mean"] = pystac.Asset(
                        href=out_data,
...
@click.option("--end_hour", default="23:00:00")

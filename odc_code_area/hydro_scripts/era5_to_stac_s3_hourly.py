...
def cds_netcdf(
    ...
    for year in years:
        for month in months:
            ...
            # print(era_var, year, month, lst_days, hours)
            # lst_cds.append(cds_response(era_var, year, month, lst_days, hours))

            # make individual calls for each hour of each day
            for dia in lst_days:
                for h in hours:
                    lst_cds.append(cds_response(era_var, year, month, dia, h))
                    break
                break
...
@click.option("--start_hour", default="01:00:00")

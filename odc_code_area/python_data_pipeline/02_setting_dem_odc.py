...
def dem_catchm_values(wrk_path:str, dc_array, 
    ...
    _tif = Path(wrk_path / 'shetran_data/temp_data/dem_cop.tif')
...
def shetran_dem(df_geo, crs_lcl, wrk_path):
    ...
    utils.shetran_csv_file(wrk_path, 'final_dem_min_SHETRAN', df_pivot_min, 'd')
    log.info(f'\n --- \n SHETRAN min elevation file created\n --- \n')

    utils.shetran_csv_file(wrk_path, 'final_dem_mean_SHETRAN', df_pivot_mean, 'd')
...
def dem_map():
    ...
    # Creating frinal map
    shetran_dem(grid_DEM, crs_local, dir_abs)

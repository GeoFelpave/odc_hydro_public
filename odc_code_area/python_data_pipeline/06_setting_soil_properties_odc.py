...
def soil_catchm_values(file_name, dc_array, 
    ...
    _tif = Path(wrk_path / f'shetran_data/temp_data/{file_name}.tif')
    ...
    # delte temp .tif file
    utils.file_remove(Path(wrk_path / 'shetran_data/temp_data/'), 'tif')
...
def project_file(df_soil_types_category, wrk_path):
    ...
    dict_tags = {
        ...
        'DEMMeanFileName':'final_dem_mean_SHETRAN.txt',
        'DEMminFileName':'final_dem_min_SHETRAN.txt',
    ...
    with open(Path(wrk_path / f"shetran_data/model_input/catch_project_file.xml"), "w") as f:
        f.write(treestring)
...
def subsurface_map(df_soil_types_category, wrk_path,
    ...
    utils.shetran_csv_file(wrk_path, 'final_subsurface_SHETRAN', df_subsurface_pivot, 'd')
